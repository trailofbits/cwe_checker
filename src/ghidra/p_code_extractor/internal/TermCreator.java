...
public class TermCreator {
     ...
     * 
     *          Creates the project term with an unique TID and adds external
     *          symbols.
     */
     ...
     * @return: new Sub Term
     * 
     *          Creates a Sub Term with an unique TID consisting of the prefix sub
     ...
     * @return: new Blk Term
     * 
     *          Creates a Blk Term with an unique TID consisting of the prefix blk
     *          and its entry address.
     */
     ...
     * @return: new Jmp Term
     * 
     *          Creates a Jmp Term with an unique TID consisting of the prefix jmp,
     *          its instruction address and the index of the pcode in the block.
     *          Depending on the instruction, it either has a goto label, a goto
     *          label and a condition or a call object.
     */
    public static ArrayList<Term<Jmp>> createJmpTerm(Boolean intraJump) {
        ...
        case PcodeOp.CALLOTHER:
            ...
            jumps.add(new Term<Jmp>(jmpTid, new Jmp(ExecutionType.JmpType.CALL,
                    HelperFunctions.resolveCallMenmonic(call, mnemonic), call, PcodeBlockData.pcodeIndex)));
            break;
        case PcodeOp.UNIMPLEMENTED:
            jumps.add(new Term<Jmp>(jmpTid,
                    new Jmp(ExecutionType.JmpType.CALL, "CALLOTHER", createCall(), PcodeBlockData.pcodeIndex)));
            break;
        case PcodeOp.CBRANCH:
            return handleConditionalBranches(jmpTid, intraJump);
        case PcodeOp.BRANCH:
            jumps.add(new Term<Jmp>(jmpTid,
                    new Jmp(ExecutionType.JmpType.GOTO, mnemonic, createLabel(null), PcodeBlockData.pcodeIndex)));
            break;
        ...
        case PcodeOp.RETURN:
            jumps.add(new Term<Jmp>(jmpTid,
                    new Jmp(ExecutionType.JmpType.RETURN, mnemonic, createLabel(null), PcodeBlockData.pcodeIndex)));
            break;
        }
     ...
     * 
     *          Creates a jump term for an indirect jump and adds jump targets as
     *          computed by Ghidra to it as target hints.
     */
    public static Jmp createIndirectJump(String mnemonic) {
        ...
        for (Reference reference : PcodeBlockData.instruction.getReferencesFrom()) {
            if (reference.getReferenceType() == RefType.COMPUTED_JUMP
                    || reference.getReferenceType() == RefType.CONDITIONAL_COMPUTED_JUMP) {
     ...
     * 
     *          Creates jmp terms for for a cbranch and an artificial fallthrough
     *          branch. It checks whether the CBRANCH occured inside a pcode block.
     *          If so, the target TID for the fall through BRANCH is set to the
     *          artificially generated block at the same address with the start
     *          pcode index of the next block.
     */
    private static ArrayList<Term<Jmp>> handleConditionalBranches(Tid conditionalTid, Boolean intraJump) {
        ...
        Tid branchTid = new Tid(String.format("instr_%s_%s", branchSiteAddress, PcodeBlockData.pcodeIndex + 1),
                branchSiteAddress);
        Tid targetTid = new Tid();

        if (intraJump) {
            targetTid = new Tid(String.format("blk_%s_%s", branchSiteAddress, PcodeBlockData.pcodeIndex + 2),
                    branchSiteAddress);
        } else {
            targetTid = new Tid(String.format("blk_%s", PcodeBlockData.instruction.getFallThrough().toString()),
                    PcodeBlockData.instruction.getFallThrough().toString());
        }

        branches.add(new Term<Jmp>(conditionalTid,
                new Jmp(ExecutionType.JmpType.GOTO, PcodeBlockData.pcodeOp.getMnemonic(), TermCreator.createLabel(null),
                        TermCreator.createVariable(PcodeBlockData.pcodeOp.getInput(1)), PcodeBlockData.pcodeIndex)));
        branches.add(new Term<Jmp>(branchTid,
                new Jmp(ExecutionType.JmpType.GOTO, "BRANCH", new Label(targetTid), PcodeBlockData.pcodeIndex + 1)));
     ...
     * @return: new Def Term
     * 
     *          Creates a Def Term with an unique TID consisting of the prefix def,
     *          its instruction address and the index of the pcode in the block.
     */
    public static Term<Def> createDefTerm() {
        Address instrAddr = PcodeBlockData.instruction.getAddress();
        Tid defTid = new Tid(String.format("instr_%s_%s", instrAddr.toString(), PcodeBlockData.pcodeIndex),
                instrAddr.toString());
        ...
        return new Term<Def>(defTid, new Def(createVariable(PcodeBlockData.pcodeOp.getOutput()), createExpression(),
     ...
     * @return: new Variable
     * 
     *          Set register name based on being a register, virtual register,
     *          constant or ram address. In case it is a virtual register, prefix
     *          the name with $U. In case it is a constant, remove the const prefix
     *          from the constant.
     */
     ...
     * 
     *          Create an Expression using the input varnodes of the pcode
     *          instruction.
     */
     ...
     * @return: new Label
     * 
     *          Create a Label based on the branch instruction. For indirect
     *          branches and calls, it consists of a Variable, for calls of a sub
     *          TID and for branches of a blk TID.
     */
    public static Label createLabel(Address fallThrough) {
        ...
        if (fallThrough == null) {
            ...
            case PcodeOp.UNIMPLEMENTED:
                ...
                break;
            default:
                jumpLabel = new Label((Tid) new Tid(
                        String.format("blk_%s", PcodeBlockData.pcodeOp.getInput(0).getAddress().toString()),
                        PcodeBlockData.pcodeOp.getInput(0).getAddress().toString()));
                break;
            }
            return jumpLabel;
        }
     ...
     * 
     *          Either returns an address to the memory if not resolved or an
     *          address to a symbol
     */
    private static Label handleLabelsForCalls(PcodeOp pcodeOp) {
        ...
        if (pcodeOp.getOpcode() == PcodeOp.CALL) {
            return new Label(new Tid(String.format("sub_%s", pcodeOp.getInput(0).getAddress().toString()),
                    pcodeOp.getInput(0).getAddress().toString()));
        }
    ...
    private static Tid getTargetTid(PcodeOp pcodeOp) {
        ...
        if (flowDestinations.length == 1) {
            ...
            // In some cases indirect calls do not follow addresses directly but contents of
            // registers
            ...
            // Also, since the function pointer address is not already in the entry points
            // map
            // the sub TID of the corresponding external symbol will be swapped with the
            // function
            // pointer address.
    ...
    private static Call createCall() {
        ...
        if (PcodeBlockData.pcodeOp.getOpcode() == PcodeOp.CALLOTHER) {
            callString = HelperFunctions.ghidraProgram.getLanguage()
                    .getUserDefinedOpName((int) PcodeBlockData.pcodeOp.getInput(0).getOffset());
